...
static uint8_t pool_cmd_buf[ OS_MEMPOOL_BYTES(POOL_CMD_COUNT, POOL_CMD_SIZE) ] __attribute__((aligned(4)));
static struct os_mempool pool_cmd;

static uint8_t pool_evt_buf[ OS_MEMPOOL_BYTES(POOL_EVT_COUNT, POOL_EVT_SIZE) ] __attribute__((aligned(4)));
static struct os_mempool pool_evt;

static uint8_t pool_evt_lo_buf[ OS_MEMPOOL_BYTES(POOL_EVT_LO_COUNT, POOL_EVT_SIZE) ] __attribute__((aligned(4)));
static struct os_mempool pool_evt_lo;

#if POOL_ACL_COUNT > 0
static uint8_t pool_acl_buf[ OS_MEMPOOL_BYTES(POOL_ACL_COUNT, POOL_ACL_SIZE) ] __attribute__((aligned(4)));
static struct os_mempool_ext pool_acl;
static struct os_mbuf_pool mpool_acl;
#endif

#if POOL_ISO_COUNT > 0
static uint8_t pool_iso_buf[ OS_MEMPOOL_BYTES(POOL_ISO_COUNT, POOL_ISO_SIZE) ] __attribute__((aligned(4)));
...
ble_transport_free(void *buf)
{
    if (os_memblock_from(&pool_cmd, buf)) {
        ...
        hci_ipc_put(HCI_IPC_TYPE_EVT_DISCARDABLE);
			...
			//TODO: This mem is from ble controller need to check
        //assert(0);
    }
}
