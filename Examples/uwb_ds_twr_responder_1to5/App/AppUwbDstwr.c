...
#define DEF_DSTWR_SYNC_RX_RESTART_TIMEOUT_MS    100
...
{  
  ...
  cb_uwbsystem_txpayload_st stchipifTxPack = {0};
  ...
  uint32_t chipID = cb_efuse_read_chip_id()>>32;
  srand(chipID);
  uint8_t delay_timer = rand()%100 + 1;
  
//  app_uwb_dstwr_print("delay timer:%d,ID:%x\n",delay_timer,chipID);
...
//           app_uwb_dstwr_print("receive\n");
           ...
           {  
               ...
               cb_framework_uwb_get_rx_payload(&message[0], 4);
            //   app_uwb_dstwr_print("%x %x %x %x\n",message[0],message[1],message[2],message[3]);
               if(message[0] == 'S' && message[1] == 'C' && message[2] == 'A' && message[3] == 'N')//scan message for initor
               {                  
                  uint8_t chipid[6] = {0x55,0xaa,chipID>>24,chipID>>16,chipID>>8,chipID};
                  stchipifTxPack.ptrAddress = &chipid[0];
                  stchipifTxPack.payloadSize = 6;
                  cb_hal_delay_in_us(delay_timer * 300);
                  cb_framework_uwb_tx_start(&s_stUwbPacketConfig, &stchipifTxPack, &stTxIrqEnable, EN_TRX_START_NON_DEFERRED); 
                //  app_uwb_dstwr_print("TX CHIPID\n");
           ...
           {
               app_uwb_dstwr_print("again\n");
               cb_framework_uwb_rx_end(EN_UWB_RX_0);
               cb_framework_uwb_rx_start(EN_UWB_RX_0, &s_stUwbPacketConfig, &stRxIrqEnable, EN_TRX_START_NON_DEFERRED); 
           }               
           
        }
        if(cb_hal_is_time_elapsed_ms(startTime, 100))
        {
//          app_uwb_dstwr_print("rx again\n");
          s_enAppDstwrState = EN_APP_STATE_IDLE;
          cb_framework_uwb_rx_end(EN_UWB_RX_0);
        }
        break;
      ...
      case EN_APP_STATE_SYNC_WAIT_RX_DONE:
        if (cb_hal_is_time_elapsed_ms(startTime, DEF_DSTWR_SYNC_RX_RESTART_TIMEOUT_MS))
        {
          s_enAppDstwrState = EN_APP_STATE_IDLE;
          cb_framework_uwb_rx_end(EN_UWB_RX_0);
        }
      ...
      {
        if (cb_hal_is_time_elapsed_ms(startTime, DEF_DSTWR_RESULT_WAIT_TIME_MS))
        {
      ...
      case EN_APP_STATE_RESULT_WAIT_TX_DONE:
      {
        if (s_stIrqStatus.TxDone == APP_TRUE)
        {
          ...
          cb_framework_uwb_tx_end();
          cb_framework_uwb_rx_start(EN_UWB_RX_0, &s_stUwbPacketConfig, &stRxIrqEnable, EN_TRX_START_NON_DEFERRED);
          s_enAppDstwrState = EN_APP_STATE_DISTANCE_WAIT_RX_DONE;
        }
        break;
      }
      ...
      case EN_APP_STATE_TERMINATE:
      {
        ...
        s_enAppDstwrState = EN_APP_STATE_IDLE;
//        uart_wait_tx_idle();  
//        cb_deep_sleep_control(70);
//        app_uwb_dstwr_print("wake up\n");
        break;
      }
    }
  }  
...
uint8_t app_dstwr_validate_sync_payload(void)
{
  ...
  uint32_t chipID = cb_efuse_read_chip_id()>>32;
  s_syncExpectedRxPayload[0] = (chipID>>24) & 0xff;
  s_syncExpectedRxPayload[1] = (chipID>>16) & 0xff;
  s_syncExpectedRxPayload[2] = (chipID>>8) & 0xff;
  s_syncExpectedRxPayload[3] = (chipID) & 0xff;
  ...
  if (rxStatus.rx0_ok == CB_TRUE)
  {
    ...
    {
     //  app_uwb_dstwr_print("c:%x %x\n",syncRxPayload[i],s_syncExpectedRxPayload[i]);
...
void app_dstwr_log(void) 
{
  ...
  {
      uint32_t chipID = cb_efuse_read_chip_id()>>32;
      app_uwb_dstwr_print("Cycle:%u D:%d cm ID:%x:%x:%x:%x\n", s_appCycleCount++, distance, (chipID >> 24) & 0xff, (chipID >> 16) & 0xff, (chipID >> 8) & 0xff, chipID & 0xff); 
  }
  else
  {
