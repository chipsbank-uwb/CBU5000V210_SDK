...
{  
  ...
  app_check_chip_boot(); // flash lock
  ...
  cb_uwbsystem_systemconfig_st uwbSystemConfig = {   
	...
	#if APP_RNGAOA_ROLE
		app_uart_printf("uwb_rngaoa_ftm_dfu INITIATOR\n");
	#else
		app_uart_printf("uwb_rngaoa_ftm_dfu RESPONDER\n");
	#endif  
...
void app_check_chip_boot(void)
{
  ...
  extern enFlashStatus cb_flash_lock(void);
  cb_flash_lock();
}