...
#include "CB_efuse.h"
#include "ftm_uwb_psr.h"
#include "app_uart.h"
//-------------------------------
// DEFINE SECTION
//-------------------------------
#define FTM_LOG_ENABLE  APP_FALSE
...
uint8_t ftm_preamble_scan(uint16_t command, uint8_t *buf, uint8_t len);
uint8_t ftm_preamble_set(uint16_t command, uint8_t *buf, uint8_t len);
uint8_t ftm_preamble_get(uint16_t command, uint8_t *buf, uint8_t len);
...
void ftm_write_ranging_freq(uint16_t command, uint8_t *buf, uint8_t len);
void ftm_read_ranging_freq(uint16_t command, uint8_t *buf, uint8_t len);
void ftm_set_ranging_tx_on_off(uint16_t command, uint8_t *buf, uint8_t len);
void ftm_set_ranging_rx_on_off(uint16_t command, uint8_t *buf, uint8_t len);
void ftm_get_ranging_rx_distance(uint16_t command, uint8_t *buf, uint8_t len);

void APP_User_PreambleScanSet(uint16_t command, uint8_t *buf, uint8_t len);
void APP_User_PreambleSet(uint16_t command, uint8_t *buf, uint8_t len);
void APP_User_PreambleGet(uint16_t command, uint8_t *buf, uint8_t len);
//-------------------------------
...
static ftCmd user_command_table[] = 
{
    ...
    {0x00F5, APP_User_PreambleScanSet},
    {0x00F6, APP_User_PreambleSet},
    {0x00F7, APP_User_PreambleGet},
...
{    
    // factory command
    {0x00F0, (ftFunction)ftm_preamble_scan},
    {0x00F1, (ftFunction)ftm_preamble_set},
    {0x00F2, (ftFunction)ftm_preamble_get},
    ...
    {0x0073, ftm_set_ranging_tx_on_off},
    {0x0074, ftm_set_ranging_rx_on_off},
    {0x0075, ftm_write_ranging_freq},
    {0x0076, ftm_get_ranging_rx_distance},
    {0x0077, ftm_read_ranging_freq},
...
void APP_User_PreambleScanSet(uint16_t command, uint8_t *buf, uint8_t len)
{
  uint8_t preambleCodeIdx = ftm_preamble_scan(command, buf, len);

  if (preambleCodeIdx != 0xff)
  {
    // Reset chip
    NVIC_SystemReset();
  }
...
void APP_User_PreambleSet(uint16_t command, uint8_t *buf, uint8_t len)
{
   uint8_t statuscode = ftm_preamble_set(command, buf, len);
...
void APP_User_PreambleGet(uint16_t command, uint8_t *buf, uint8_t len)
{
  uint8_t statuscode = ftm_preamble_get(command, buf, len);
}
/**
...
uint8_t ftm_preamble_scan(uint16_t command, uint8_t *buf, uint8_t len)
{
  ...
  if (len == 3 && mode > 0)
  {
    ...
    preambleScanningParams.psrMode = EN_PSR_SINGLE_MODE; // EN_PSR_FULL_MODE;
    preambleScanningParams.scanDuration = duration;      // 200;
    app_uwb_psr_init(preambleScanningParams);

    psr_app_uwb_irq_callback_init(); // IRQ ISR register
    preambleCodeIdx = app_uwb_psr_sequence();
    psr_app_uwb_irq_callback_deinit(); // IRQ ISR deregister
  }
  else
  {
    APP_UWB_PSR_Suspend();
...
void ftm_read_chip_id(uint16_t command, uint8_t *buf, uint8_t len)
{
  ...
  uint64_t chipid = cb_efuse_read_chip_id();
  LOG("chipid: %llx \n", chipid);
...
void ftm_read_aoa_cal(uint16_t command, uint8_t *buf, uint8_t len)
{
  ...
  LOG("AoaCalValue, index: %x, pdoa01: %x, pdoa02: %x, pdoa12: %x , pdoa2: %x, \n",
      index, aoacal1.Pdoa01, aoacal1.Pdoa02, aoacal1.Pdoa12, aoacal1.Calpdoa2);

  stCaliAoa calAoa;
  calAoa.Pdoa01 = BigLittleSwap16(aoacal1.Pdoa01);
  calAoa.Pdoa02 = BigLittleSwap16(aoacal1.Pdoa02);
  calAoa.Pdoa12 = BigLittleSwap16(aoacal1.Pdoa12);
...
void ftm_write_aoa_cal(uint16_t command, uint8_t *buf, uint8_t len)
{
  ...
  stCaliAoa aoacal1;
  uint8_t index = buf[0];
  aoacal1.Pdoa01 = (int16_t)buf[1] << 8 | buf[2];
  aoacal1.Pdoa02 = (int16_t)buf[3] << 8 | buf[4];
  aoacal1.Pdoa12 = (int16_t)buf[5] << 8 | buf[6];
...
void ftm_get_rang_aoa_rx_result(uint16_t command, uint8_t *buf, uint8_t len)
{
  ...
  LOG("PDOAH:%d\n", rxResultData.pdoah);
  LOG("PDOAV:%d\n", rxResultData.pdoav);
  LOG("AVG_RSL1:%d\n", rxResultData.avg_rssi1);
  LOG("AVG_RSL2:%d\n", rxResultData.avg_rssi2);
  LOG("AVG_RSL3:%d\n", rxResultData.avg_rssi3);
  LOG("PD01:%d\n", rxResultData.pd01);
  LOG("PD02:%d\n", rxResultData.pd02);
  LOG("PD12:%d\n", rxResultData.pd12);
  //	LOG("DIS: %d, AOAH: %d, AOAV: %d, PDOAH: %d, PDOAV: %d, AVG_RSL1: %d, AVG_RSL2: %d, AVG_RSL3: %d, PD01: %d , PD02: %d, PD12: %d\n", \
//		rxResultData.dis,rxResultData.aoah, rxResultData.aoav, rxResultData.pdoah, rxResultData.pdoav,rxResultData.avg_rsl1,rxResultData.avg_rsl2,rxResultData.avg_rsl3,rxResultData.pd01,rxResultData.pd02,rxResultData.pd12);
  ...
  rxResultData.avg_rssi1 = BigLittleSwap16(rxResultData.avg_rssi1);
  rxResultData.avg_rssi2 = BigLittleSwap16(rxResultData.avg_rssi2);
  rxResultData.avg_rssi3 = BigLittleSwap16(rxResultData.avg_rssi3);
  rxResultData.pd01 = BigLittleSwap16(rxResultData.pd01);
  rxResultData.pd02 = BigLittleSwap16(rxResultData.pd02);
  rxResultData.pd12 = BigLittleSwap16(rxResultData.pd12);
...
void ftm_write_ranging_freq(uint16_t command, uint8_t *buf, uint8_t len)
{
  ...
  uint8_t freq = *buf;
  uint8_t statuscode = ftm_uwb_cal_set_ranging_freq(freq);
  LOG("ftm_uwb_cal_set_ranging_freq return(0:ok, 1:fail): %d \n", statuscode);
  LOG("Ranging freq: %d \n", freq);
...
void ftm_read_ranging_freq(uint16_t command, uint8_t *buf, uint8_t len)
{
  ...
  uint8_t freq = 0;
  uint8_t statuscode = ftm_uwb_cal_get_ranging_freq(&freq);
  LOG("ftm_uwb_cal_get_ranging_freq return(0:ok, 1:fail): %d \n", statuscode);
  LOG("Ranging freq: %d \n", freq);
...
void ftm_set_ranging_tx_on_off(uint16_t command, uint8_t *buf, uint8_t len)
{
  ...
  uint8_t respondLen = sizeof(statuscode);
  ftm_command_respond(command,&statuscode,respondLen);
  ftm_uwb_cal_set_ranging_tx_onoff(OnOff);
  LOG("ftm_uwb_cal_set_ranging_tx_onoff return(0:ok, 1:fail): %d \n", statuscode);
  LOG("Ranging Tx OnOff: %d \n", OnOff);
}
...
void ftm_set_ranging_rx_on_off(uint16_t command, uint8_t *buf, uint8_t len)
{
  ...
  uint8_t respondLen = sizeof(statuscode);
  ftm_command_respond(command,&statuscode,respondLen);
  ftm_uwb_cal_set_ranging_rx_onoff(OnOff);
  LOG("ftm_uwb_cal_set_ranging_rx_onoff return(0:ok, 1:fail): %d \n", statuscode);
  LOG("Ranging Rx OnOff: %d \n", OnOff);
}
...
void ftm_get_ranging_rx_distance(uint16_t command, uint8_t *buf, uint8_t len)
{
  ...
  uint8_t statuscode = ftm_uwb_cal_get_ranging_distance(&distanceCm);
  LOG("ftm_uwb_cal_get_ranging_distance return(0:ok, 1:fail): %d \n", statuscode);
  LOG("Ranging distance:%dcm\n", distanceCm);

  distanceCm = BigLittleSwap32(distanceCm);
  uint8_t respondLen = sizeof(distanceCm);
  ftm_command_respond(command,(uint8_t*)&distanceCm,respondLen);
}
