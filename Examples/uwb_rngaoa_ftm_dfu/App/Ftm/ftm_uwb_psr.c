...
//-------------------------------
#define APP_UWB_PSR_UARTPRINT_ENABLE APP_FALSE
...
//-------------------------------
void app_psr_start(uint16_t duration)
{
...
cb_uwbsystem_preamblecodeidx_en app_uwb_psr_sequence(void)
{
  ...
  if(s_PreambleScanningParams.trxMode == EN_PSR_RX)
  {
     ...
     CB_TRNG_GetRng(&rn, 1);
     uint8_t start_idx= rn%(PREAMBLE_CODE_IDX_SIZE);
	 ...
	 for(uint8_t i = PREAMBLE_CODE_IDX_MIN; i <= PREAMBLE_CODE_IDX_MAX; ++i)
	 {
        app_uwb_psr_print(".");
        #if 1
        uint8_t rn_idx = start_idx+i;
        if(rn_idx > PREAMBLE_CODE_IDX_MAX)
        {
            rn_idx = PREAMBLE_CODE_IDX_MIN+(rn_idx - PREAMBLE_CODE_IDX_MAX-1);
        }
        
        cb_system_set_preamble_index(rn_idx);
        #else
        cb_system_set_preamble_index(i);
        #endif
		
		uint32_t  startTime = cb_hal_get_time_ms();		    
		...
		while(1)
		{
		   while(s_IrqStatus.Rx0Done == APP_FALSE)
		   {
				...
				if((cb_hal_is_time_elapsed_ms(startTime,s_PreambleScanningParams.scanDuration) == CB_PASS)) // 200ms
				{
 ...
 */
void app_psr_uwbapp_rx0_preamble_detected_irqcb(void)
{
 ...
 */
void app_psr_uwbapp_rx0_sfd_detected_irqcb(void)
{
 ...
 */
void app_psr_uwbapp_rx0_done_irqcb(void)
{
  s_IrqStatus.Rx0Done = APP_TRUE;
}

void psr_app_uwb_irq_callback_init(void)
{
  app_irq_register_irqcallback(EN_IRQENTRY_UWB_RX0_DONE_APP_IRQ         ,app_psr_uwbapp_rx0_done_irqcb);
  app_irq_register_irqcallback(EN_IRQENTRY_UWB_RX0_PD_DONE_APP_IRQ         ,app_psr_uwbapp_rx0_preamble_detected_irqcb);
  app_irq_register_irqcallback(EN_IRQENTRY_UWB_RX0_SFD_DET_DONE_APP_IRQ         ,app_psr_uwbapp_rx0_sfd_detected_irqcb);
}

void psr_app_uwb_irq_callback_deinit(void)
{
  app_irq_deregister_irqcallback(EN_IRQENTRY_UWB_RX0_DONE_APP_IRQ         ,app_psr_uwbapp_rx0_done_irqcb);
  app_irq_deregister_irqcallback(EN_IRQENTRY_UWB_RX0_PD_DONE_APP_IRQ         ,app_psr_uwbapp_rx0_preamble_detected_irqcb);
  app_irq_deregister_irqcallback(EN_IRQENTRY_UWB_RX0_SFD_DET_DONE_APP_IRQ         ,app_psr_uwbapp_rx0_sfd_detected_irqcb);
}
