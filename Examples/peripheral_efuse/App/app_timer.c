...
void app_timer_main(void)
{
    ...
    timer_setup.stTimeOut.timeoutVal[0]        = 10;                        // Set the timeout value to 100 ms
    ...
    app_irq_register_irqcallback(EN_IRQENTRY_TIMER_0_APP_IRQ, app_timer_0_irq_callback);
    cb_timer_init(EN_TIMER_0);
    cb_timer_configure_timer(&timer_setup);
	
}

/**
 * @brief   Callback function for Timer 0 IRQ.
 * @param   TimeOutEventNumber The timeout event number associated with the IRQ.
 */
 ...
 * 3. Initializes GPIO.
 * 4. Configures and starts the Timer.
 * 5. Prints a power-on reset message via UART.
 */
