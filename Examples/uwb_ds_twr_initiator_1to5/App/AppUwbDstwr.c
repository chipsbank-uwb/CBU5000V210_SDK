...
#define SUPPORT_MAX_DEVICE 5

typedef struct
{
   uint8_t send_mac[4];
   uint8_t device_mac[SUPPORT_MAX_DEVICE * 4];
   uint8_t device_total_num ;
  
}device_dstwr_t;

device_dstwr_t device_dstwr;
//-------------------------------
...
typedef enum{
  ...
  //IDLE STATE
  EN_APP_STATE_IDLE = 0,
  EN_APP_STATE_SCAN_DEVICE,
  EN_APP_STATE_RECEIVE_DEVICE_RESPONS,
  EN_APP_STATE_WAIT_DEVICE,
  //SYNC STATE
...
static cb_uwbsystem_packetconfig_st s_stUwbPacketConfig = 
{
  ...
  .preambleCodeIndex  = EN_UWB_PREAMBLE_CODE_IDX_20,       // Preamble code index (9-32)
...
static uint8_t s_scanTxPayload[4] = {'S','C','A','N'};
...
{  
  ...
  cb_uwbsystem_txpayload_st stScanTxPayloadPack = {0};
...
//  stSyncTxPayloadPack.payloadSize   = sizeof(s_syncTxPayload);
  ...
  stScanTxPayloadPack.ptrAddress = &s_scanTxPayload[0];
  stScanTxPayloadPack.payloadSize = sizeof(s_scanTxPayload);
  //--------------------------------
  // Configure IRQ
  //--------------------------------
  // TX IRQ
  ...
  memset(&device_dstwr,0,sizeof(device_dstwr));
  ...
  while(1)
  {
    ...
    switch (s_enAppDstwrState)
    {
      ...
      case EN_APP_STATE_IDLE:
        if(device_dstwr.device_total_num)
        {
           cb_system_delay_in_us(10);
           s_enAppDstwrState = EN_APP_STATE_SYNC_TRANSMIT;
            
        }
        else
        {
           s_enAppDstwrState = EN_APP_STATE_SCAN_DEVICE;
        
        }
        break;
  
      case EN_APP_STATE_SCAN_DEVICE:
        cb_system_delay_in_us(100);
        memset(&device_dstwr,0,sizeof(device_dstwr));
        cb_framework_uwb_tx_start(&s_stUwbPacketConfig, &stScanTxPayloadPack, &stTxIrqEnable, EN_TRX_START_NON_DEFERRED);  
        s_enAppDstwrState = EN_APP_STATE_RECEIVE_DEVICE_RESPONS;      
        break;
      
      case EN_APP_STATE_RECEIVE_DEVICE_RESPONS:
        if(s_stIrqStatus.TxDone == APP_TRUE)
        {
          ...
          s_enAppDstwrState  = EN_APP_STATE_WAIT_DEVICE;
     
        }  
        break;
        
      case EN_APP_STATE_WAIT_DEVICE:
...
//               cb_framework_uwb_rx_end(EN_UWB_RX_0);
               cb_framework_uwb_get_rx_payload(&mac[0], 6);
               if(mac[0] == 0x55 && mac[1] == 0xaa)
               {
                  memcpy(&device_dstwr.device_mac[device_dstwr.device_total_num*4],&mac[2],4); 
                  device_dstwr.device_total_num++;  
                  if(SUPPORT_MAX_DEVICE == device_dstwr.device_total_num) 
                  {
                     s_enAppDstwrState = EN_APP_STATE_SYNC_TRANSMIT;  
                  }
//                  app_uwb_dstwr_print("device:%d\n",device_dstwr.device_total_num);   
        ...
        else if(cb_hal_is_time_elapsed_ms(startTime, 30))
        {
           cb_framework_uwb_rx_end(EN_UWB_RX_0);
           app_uwb_dstwr_print("scan time out:%d\n",device_dstwr.device_total_num);
           if(device_dstwr.device_total_num)
           {
              s_enAppDstwrState = EN_APP_STATE_SYNC_TRANSMIT;     
           }
...
//        for(int i=0;i<device_dstwr.device_total_num*4;i++)
//        {
//          app_uwb_dstwr_print("%x ",device_dstwr.device_mac[i]);
//        }
//        app_uwb_dstwr_print("\n");
        device_dstwr.device_total_num--;
        memcpy(device_dstwr.send_mac,&device_dstwr.device_mac[device_dstwr.device_total_num * 4],4);
        stSyncTxPayloadPack.ptrAddress = &device_dstwr.send_mac[0];
        stSyncTxPayloadPack.payloadSize = 4;
      ...
      case EN_APP_STATE_SYNC_WAIT_RX_DONE:
        if (cb_hal_is_time_elapsed_ms(startTime, DEF_DSTWR_SYNC_ACK_TIMEOUT_MS))
        {
      ...
      case EN_APP_STATE_RESULT_WAIT_RX_DONE:
        ...
        {        
          ...
          cb_framework_uwb_rx_end(EN_UWB_RX_0);
          s_enAppDstwrState = EN_APP_STATE_DISTANCE_WAIT_TX_DONE;
        }
        break;
      ...
      case EN_APP_STATE_DISTANCE_WAIT_TX_DONE:
        {
         ...
         //  app_uwb_rngaoa_print("tx distance\n");
        }
        break;
      //-------------------------------------
      // Terminate
      ...
      case EN_APP_STATE_TERMINATE:
        if(s_stIrqStatus.TxDone == APP_TRUE || s_applicationTimeout)
        {
           s_stIrqStatus.TxDone = APP_FALSE;
...
void app_dstwr_log(void) 
{
  ...
  {
    uint32_t  distance = (uint32_t) s_measuredDistance;
      app_uwb_dstwr_print("Cycle:%u, D:%d cm ID:%x:%x:%x:%x\n", s_appCycleCount++, distance,device_dstwr.device_mac[device_dstwr.device_total_num*4],\
      device_dstwr.device_mac[device_dstwr.device_total_num*4+1],device_dstwr.device_mac[device_dstwr.device_total_num*4+2],device_dstwr.device_mac[device_dstwr.device_total_num*4+3]);      
  }
  else
  {
