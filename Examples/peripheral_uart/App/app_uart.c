...
{
    cb_scr_uart0_module_on(); // Turn on the UART0 module
    cb_iomux_config(EN_IOMUX_GPIO_0, &(stIomuxGpioMode){EN_IOMUX_GPIO_MODE_SOC_PERIPHERALS, (uint8_t)EN_IOMUX_GPIO_AF_UART0_RXD}); // Configure GPIO 0 for UART0 RXD
    cb_iomux_config(EN_IOMUX_GPIO_1, &(stIomuxGpioMode){EN_IOMUX_GPIO_MODE_SOC_PERIPHERALS, (uint8_t)EN_IOMUX_GPIO_AF_UART0_TXD}); // Configure GPIO 1 for UART0 TXD
          
    NVIC_EnableIRQ(UART0_IRQn); // Enable UART0 interrupt   

    uart_config.uartChannel     = EN_UART_0;                    // Set UART channel to UART0
    uart_config.uartMode        = EN_UART_MODE_SDMA;            // Set UART mode to SDMA
    uart_config.uartBaudrate    = EN_UART_BAUDRATE_115200;      // Set UART baud rate to 115200
    uart_config.uartRxMaxBytes  = 1;                            // Set maximum number of bytes to receive at once to 1
    uart_config.uartRxBufWrap   = EN_UART_RXBUF_WRAP_DISABLE;   // Disable receive buffer wrapping
    uart_config.uartStopBits    = EN_UART_STOP_BITS_1;          // Set number of stop bits to 1
    uart_config.uartBitOrder    = EN_UART_BIT_ORDER_LSB_FIRST;  // Set bit order to LSB first
...
void app_uart_change_baudrate(enUartBaudrate baudrate)
{
    ...
    /* Callback IRQ register omitted in current SDK to improve IRQ processing time */
    // app_irq_deregister_irqcallback(EN_IRQENTRY_UART_0_RXB_FULL_APP_IRQ, app_uart_0_rxb_full_callback);
